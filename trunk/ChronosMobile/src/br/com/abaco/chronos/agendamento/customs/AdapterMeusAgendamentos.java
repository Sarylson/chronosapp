 ...
 */
public class AdapterMeusAgendamentos extends ArrayAdapter<MeusAgendamentosItem> {
	...
	public AdapterMeusAgendamentos( final Context context, final int resourceId, final List<MeusAgendamentosItem> listItems ) {
	...
	public View getView( final int posicao, View viewConverter, final ViewGroup group ) {
		...
		final MeusAgendamentosItem itemCustom = getItem( posicao );
		...
		if ( viewConverter == null ) {
			...
			viewConverter = layoutInflater.inflate( R.layout.listview_custom_meus_agendamentos, null );
